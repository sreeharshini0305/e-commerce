    ...
    return (
        <div className={`${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
            <div className='flex items-centre justify-between py-6 border-b'>
                <div className='uppercase text-medium font-semibold justify-center items-center'>Shopping Bag({itemAmount})</div>
            {/* icon */}
            <div onClick={handleClose} className='cursor-pointer w-8 h-8 flex justify-center items-center'>
                <IoMdArrowForward className='text-2xl' />
            </div>
            </div>
            <div className='flex flex-col gap-y-2 h-[500px] lg:h-[500px] overflow-y-auto overflow-x-hidden border-b'>
                {cart.map((item)=>{
            ...
            <div className='flex flex-col gap-y-3 mt-4'>
                ...
                <Link to='/' className='bg-black flex p-4 justify-center items-center text-white w-full font-medium'>Check out</Link>
            </div>
</div>
    )
}
